...
from .routers import tasks, calendar, focus, alarms, habits, boards, review, commands, player
...
async def request_context(request: Request, call_next):
    ...
    if os.getenv('ZOESTM_ENFORCE_AUTH', '0') == '1':
        public_paths = {'/health', '/health/detail', '/meta/openapi', '/meta/version', '/metrics'}
        if request.url.path not in public_paths and not request.headers.get('x-token-scopes'):
            payload = error_payload('auth_required', 'Missing authentication scopes header')
    ...
    if elapsed_ms > 500:
        print(json.dumps({'event': 'slow_route', 'path': request.url.path, 'elapsed_ms': elapsed_ms, 'request_id': request_id}))
    return response
...
async def http_exception_handler(request: Request, exc: HTTPException):
    ...
    if isinstance(detail, dict):
        ...
        details = detail.get('details') or {k: v for k, v in detail.items() if k not in ('code', 'message')}
    else:
        code = 'http_error'
...
def health_status():
    ...
    try:
        ...
        db_ok = False
    return {'ok': db_ok, 'dependencies': {'db': 'ok' if db_ok else 'error', 'outbox': m}}
